...
// init
const manager = FoundryLocalManager.create({
    appName: 'foundry_local_audio_tranciption',
    logLevel: 'info'
});
...
await manager.downloadAndRegisterEps((epName, percent) => {
    ...
    process.stdout.write(`\r   ${epName.padEnd(30)}   ${percent.toFixed(2).padStart(5)}%`)
});
if(currentEp !== '') process.stdout.write('\n');
...
// Create audio client
console.log('\nCreating audion client...');
const audioClient = model.createAudioClient();
console.log('✓ Audio client created');

// Example audio transiption
const audioFile = './Recording.mp3';
console.log(`\Transcribing ${audioFile}`);
const transcription = await audioClient.transcribe(audioFile);
console.log('\nAudio transcription result:');
console.log(transcription.text);
console.log('✓ Audio transcription compleated');
